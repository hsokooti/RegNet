...
def run_regnet(setting, base_learning_rate=1E-4, max_steps=1000):
    ...
    load_model = False
    network_name = 'decimation3'  # 'decimation3', 'crop3_connection'
    setting['stage'] = 1
    ...
    if setting['stage'] in [1, 2]:
        setting['NetworkTraining']['reg_weight'] = 0.01
    ...
    dvf_tf = tf.placeholder(tf.float32, shape=[None, 2*setting['Ry']+1, 2*setting['Ry']+1, 2*setting['Ry']+1, 3], name="DVF")
    ...
    huber = (tf.losses.huber_loss(dvf_tf, dvf_predict, weights=1))
    ...
    tf.summary.image('GroundTruth_X', dvf_tf[batch_summary, int(dvf_tf.get_shape()[1]) // 2, np.newaxis, :, :, 0, np.newaxis], 1)
    tf.summary.image('GroundTruth_Y', dvf_tf[batch_summary, int(dvf_tf.get_shape()[1]) // 2, np.newaxis, :, :, 1, np.newaxis], 1)
    tf.summary.image('GroundTruth_Z', dvf_tf[batch_summary, int(dvf_tf.get_shape()[1]) // 2, np.newaxis, :, :, 2, np.newaxis], 1)
    ...
    for itr in range(itr_load, max_steps):
        ...
        if itr % 25 == 1:
            ...
            if regularization:
                ...
                [s, y_plot, y_hat_plot] = sess.run([summ, dvf_tf, dvf_predict],
                                                   feed_dict={images_tf: batch_im, dvf_tf: batch_dvf, bn_training: 0,
        ...
        if itr % 1000 == 1:
                ...
                if regularization:
                    [loss_val_itr, huber_val_itr, bending_val_itr] = \
                        sess.run([loss, huber, bending_energy],
                                 feed_dict={images_tf: batch_im_val, dvf_tf: batch_dvf_val, bn_training: 0})
                    huber_val_average = huber_val_average + huber_val_itr
                    bending_val_average = bending_val_average + bending_val_itr
                else:
                    [loss_val_itr] = sess.run([loss], feed_dict={images_tf: batch_im_val, dvf_tf: batch_dvf_val, bn_training: 0})
            ...
            if regularization:
                ...
                [y_plot_val, y_hat_plot_val, s] = sess.run([dvf_tf, dvf_predict, summ],
                                                           feed_dict={images_tf: batch_im_val, dvf_tf: batch_dvf_val, bn_training: 0,
